...
export function SignupPage({ onSignup, onGoToLogin }: SignupPageProps) {
  ...
  return (
    <AuthLayout
      ...
      subtitle="Comece a criar roteiros incriveis hoje mesmo"
    >
      <form onSubmit={handleSubmit} className="flex flex-col gap-5">
        {/* Name field */}
        <div className="flex flex-col gap-2">
          <label className="text-sm text-white">Nome completo</label>
          <div className="relative">
            <User className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-[var(--oryos-text-disabled)]" />
            <Input
              type="text"
              value={name}
        ...
        <div className="flex flex-col gap-2">
          <label className="text-sm text-white">Confirmar senha</label>
          <div className="relative">
            <Lock className="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-[var(--oryos-text-disabled)]" />
            <Input
          ...
          {confirmPassword.length > 0 && password !== confirmPassword && (
            <span className="text-xs text-red-400">As senhas nao coincidem</span>
          )}
        </div>
        ...
        {/* Terms */}
        <p className="text-center text-xs text-[var(--oryos-text-disabled)] mt-2">
          Ao criar uma conta, voce concorda com nossos{" "}
          <a href="#" className="text-[var(--oryos-text-description)] hover:text-white transition-colors">
            Termos de Uso
          </a>{" "}
          e{" "}
          <a href="#" className="text-[var(--oryos-text-description)] hover:text-white transition-colors">
